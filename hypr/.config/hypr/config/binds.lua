...
-- Alterna gaps/rounding: janelas coladas nas bordas (sem wallpaper em volta) <-> normal
local gapsOn = true
hl.bind(mainMod .. " + G", function()
    if gapsOn then
        hl.config({ general = { gaps_in = 0, gaps_out = 0 }, decoration = { rounding = 0 } })
    else
        hl.config({ general = { gaps_in = 3, gaps_out = 8 }, decoration = { rounding = 10 } })
    end
    gapsOn = not gapsOn
end)
...
-- Omarchy style: SUPER + SHIFT + <n> moves the active window to workspace <n> (absolute)
for i = 1, NUM_WPM do
    local key = i % 10
...
-- Omarchy style: resize the active window with the keyboard
-- code:20 = "-"  |  code:21 = "="   (resizeactive equivalent)
...
-- NOTE: SUPER + "-" / "=" now resize the active window (Omarchy style, see above).
-- Zoom stays on the numpad +/- keys below.
...
-- Segue o navegador padrao do sistema (troque com: xdg-settings set default-web-browser <x>.desktop)
hl.bind(mainMod .. " + W",          hl.dsp.exec_cmd(launchPrefix .. "$(xdg-settings get default-web-browser)"))
...
-- Keybindings cheatsheet (estilo Omarchy)
hl.bind(mainMod .. " + ALT + K", hl.dsp.exec_cmd("/home/ericsson/.config/hypr/scripts/keybindings.sh"))

-------------------------------
---- WORKSPACES & MONITORS ----
-------------------------------

-- Focus on monitors (moved off SUPER+<n> to make room for Omarchy-style workspace keys)
...
-- Absolute -- Omarchy style: SUPER + <n> switches to workspace <n>
for i = 1, NUM_WPM do
    local key = i % 10
